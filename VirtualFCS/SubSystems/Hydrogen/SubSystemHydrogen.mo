...
model SubSystemHydrogen
  ...
  VirtualFCS.Fluid.PressureRegulator_old pressureRegulator_old annotation(
    Placement(visible = true, transformation(origin = {-32, 74}, extent = {{-14, 14}, {14, -14}}, rotation = 0)));
...
//*** DEFINE CONNECTIONS ***//
  ...
  connect(pressureRegulator_old.Output, teeJunctionIdeal.port_1) annotation(
    Line(points = {{-18, 74}, {16, 74}}, color = {0, 170, 0}, thickness = 1));
  connect(tankHydrogen.ports[1], pressureRegulator_old.Input) annotation(
    Line(points = {{-108, 74}, {-46, 74}}, color = {0, 170, 0}, thickness = 1));
  connect(setH2Pressure.y, pressureRegulator_old.setDownstreamPressure) annotation(
    Line(points = {{-26, 32}, {-26, 64}}, color = {0, 0, 127}));
  connect(sensorPressureHydrogenLine.port, pressureRegulator_old.Output) annotation(
    Line(points = {{2, 52}, {-18, 52}, {-18, 74}}));
  connect(sensorPressureTank.port, pressureRegulator_old.Input) annotation(
