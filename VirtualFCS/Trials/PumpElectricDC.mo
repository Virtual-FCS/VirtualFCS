model PumpElectricDC
  ...
  Modelica.Blocks.Interfaces.RealOutput sensors[2] annotation(
    Placement(visible = true, transformation(origin = {110, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {32, -84}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  Modelica.Electrical.Machines.BasicMachines.DCMachines.DC_PermanentMagnet dcpm(VaNominal = driveData.motorData.VaNominal, IaNominal = driveData.motorData.IaNominal, wNominal = driveData.motorData.wNominal, TaNominal = driveData.motorData.TaNominal, Ra = driveData.motorData.Ra, TaRef = driveData.motorData.TaRef, La = driveData.motorData.La, Jr = driveData.motorData.Jr, useSupport = false, Js = driveData.motorData.Js, frictionParameters = driveData.motorData.frictionParameters, coreParameters = driveData.motorData.coreParameters, strayLoadParameters = driveData.motorData.strayLoadParameters, brushParameters = driveData.motorData.brushParameters, alpha20a = driveData.motorData.alpha20a, phiMechanical(fixed = true), wMechanical(fixed = true), ia(fixed = true), TaOperational = 293.15, core(v(start = 0))) annotation(
  ...
  Modelica.Fluid.Machines.PrescribedPump pump(redeclare package Medium = Medium, redeclare function flowCharacteristic = Modelica.Fluid.Machines.BaseClasses.PumpCharacteristics.linearFlow(V_flow_nominal = {0, 0.0051333}, head_nominal = {13.05, 7.138}), N_nominal = 1200, V = 0.005, checkValve = true, energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, massDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, nParallel = 1, p_a_start = 101325, use_N_in = true) annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput control_input annotation(
    Placement(visible = true, transformation(origin = {-122, 42}, extent = {{-18, -18}, {18, 18}}, rotation = 0), iconTransformation(origin = {-24, -86}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
...
//*** DEFINE CONNECTIONS ***//
  ...
  connect(control_input, gain1.u) annotation(
    Line(points = {{-122, 42}, {-66, 42}}, color = {0, 0, 127}));
protected
  annotation(
