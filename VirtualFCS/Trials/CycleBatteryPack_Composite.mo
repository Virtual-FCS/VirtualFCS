model CycleBatteryPack_Composite "Example demonstrating constant-current constant-voltage cycling of a composite Li-ion battery pack model."
  ...
  BatteryManagementSystem BMS(N_s = liIonBatteryPack_Composite.s, lowerVoltageLimit = liIonBatteryPack_Composite.s * 3) annotation(
  ...
  LiIonBatteryPack_Composite liIonBatteryPack_Composite(SOC_init = 0.5,p = 3, s = 3)  annotation(
...
equation
  ...
  connect(pulseCurrent.n, BMS.pin_n_load) annotation(
    Line(points = {{-14, 84}, {-10, 84}, {-10, 48}}, color = {0, 0, 255}));
  connect(pulseCurrent.p, BMS.pin_p_load) annotation(
    Line(points = {{16, 84}, {12, 84}, {12, 48}}, color = {0, 0, 255}));
  annotation(
