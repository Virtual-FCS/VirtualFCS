...
model HydrogenSubsystemTEST_simplifiedV7
  ...
  Modelica.Fluid.Valves.ValveCompressible valveCompressible1(redeclare package Medium = Medium, allowFlowReversal = true, checkValve = false, dp_nominal(displayUnit = "Pa") = 10000, m_flow_nominal = 0.001, m_flow_small = 0, p_nominal = 2.5 * 100000) annotation(
    Placement(visible = true, transformation(origin = {20, 42}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
  Modelica.Blocks.Sources.RealExpression realExpression4(y = x)  annotation(
    Placement(visible = true, transformation(origin = {54, 42}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  Real x(start = 0);
equation
  
  if pressureRegulator.valveCompressible.opening <= 0.000000000001 then
    x = 1;
  else 
    x = 0;
  end if;
  ...
  connect(realExpression1.y, pressureRegulator.setDownstreamPressure) annotation(
    Line(points = {{-56, 42}, {-36, 42}, {-36, 76}}, color = {0, 0, 127}));
  connect(volume.ports[1], pressureRegulator.Input) annotation(
    Line(points = {{-68, 84}, {-50, 84}}, color = {0, 127, 255}));
  connect(pressureRegulator.Output, teeJunctionIdeal1.port_1) annotation(
    Line(points = {{-30, 84}, {-8, 84}}, color = {0, 127, 255}));
  connect(pressure1.port, pressureRegulator.Output) annotation(
    Line(points = {{-22, 68}, {-30, 68}, {-30, 84}}, color = {0, 127, 255}));
  connect(pressure.port, pressureRegulator.Input) annotation(
    Line(points = {{-50, 68}, {-50, 84}}, color = {0, 127, 255}));
  connect(recirculationBlower.Output, valveCompressible1.port_a) annotation(
    Line(points = {{22, 14}, {20, 14}, {20, 32}}, color = {0, 127, 255}));
  connect(valveCompressible1.port_b, teeJunctionIdeal1.port_3) annotation(
    Line(points = {{20, 52}, {20, 60}, {2, 60}, {2, 74}}, color = {0, 127, 255}));
  connect(realExpression4.y, valveCompressible1.opening) annotation(
    Line(points = {{44, 42}, {28, 42}}, color = {0, 0, 127}));
  annotation (experiment(StopTime = 600, Interval = 0.25, Tolerance = 1e-6));
end HydrogenSubsystemTEST_simplifiedV7;
