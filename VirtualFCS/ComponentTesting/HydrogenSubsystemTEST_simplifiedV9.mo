...
model HydrogenSubsystemTEST_simplifiedV9
  ...
  Modelica.Fluid.Vessels.ClosedVolume volume(redeclare package Medium = Medium, V = 0.13, nPorts = 1, p_start = 70000000, use_HeatTransfer = true, use_portsData = false)  annotation(
  ...
  Modelica.Fluid.Fittings.TeeJunctionIdeal teeJunctionIdeal1(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {48, 84}, extent = {{10, -10}, {-10, 10}}, rotation = -180)));
  ...
  Modelica.Fluid.Valves.ValveCompressible valveCompressible1(redeclare package Medium = Medium, allowFlowReversal = true, checkValve = true, dp_nominal(displayUnit = "Pa") = 10000, m_flow_nominal = 0.001, m_flow_small = 0, p_nominal = 2.5 * 100000) annotation(
    Placement(visible = true, transformation(origin = {20, 84}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression realExpression4(y = 0.001) annotation(
    Placement(visible = true, transformation(origin = {44, 116}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  Modelica.Fluid.Fittings.TeeJunctionIdeal teeJunctionIdeal3(redeclare package Medium = Medium) annotation(
    Placement(visible = true, transformation(origin = {-6, 84}, extent = {{-8, 8}, {8, -8}}, rotation = 0)));
  Modelica.Fluid.Vessels.ClosedVolume volume1(redeclare package Medium = Medium, V = 0.005, nPorts = 1, p_start = system.p_start, use_HeatTransfer = true, use_portsData = false)  annotation(
    Placement(visible = true, transformation(origin = {-1, 59}, extent = {{-7, -7}, {7, 7}}, rotation = 180)));
equation
  ...
  connect(teeJunctionIdeal1.port_2, teeJunctionIdeal.port_1) annotation(
    Line(points = {{58, 84}, {62, 84}, {62, 66}, {112, 66}, {112, -136}, {-26, -136}, {-26, -114}, {-10, -114}}, color = {0, 127, 255}));
  ...
  connect(recirculationBlower.Output, teeJunctionIdeal1.port_3) annotation(
    Line(points = {{22, 14}, {22, 44}, {48, 44}, {48, 74}}, color = {0, 127, 255}));
  connect(volume.ports[1], pressureRegulator.Input) annotation(
    Line(points = {{-68, 84}, {-46, 84}}, color = {0, 127, 255}));
  connect(pressure1.port, pressureRegulator.Output) annotation(
    Line(points = {{-22, 68}, {-26, 68}, {-26, 84}}, color = {0, 127, 255}));
  connect(pressure.port, pressureRegulator.Input) annotation(
    Line(points = {{-50, 68}, {-46, 68}, {-46, 84}}, color = {0, 127, 255}));
  connect(realExpression1.y, pressureRegulator.setDownstreamPressure) annotation(
    Line(points = {{-56, 42}, {-32, 42}, {-32, 76}}, color = {0, 0, 127}));
  connect(valveCompressible1.port_b, teeJunctionIdeal1.port_1) annotation(
    Line(points = {{28, 84}, {38, 84}}, color = {0, 127, 255}));
  connect(realExpression4.y, valveCompressible1.opening) annotation(
    Line(points = {{34, 116}, {20, 116}, {20, 90}}, color = {0, 0, 127}));
  connect(pressureRegulator.Output, teeJunctionIdeal3.port_1) annotation(
    Line(points = {{-26, 84}, {-14, 84}}, color = {0, 127, 255}));
  connect(teeJunctionIdeal3.port_2, valveCompressible1.port_a) annotation(
    Line(points = {{2, 84}, {12, 84}}, color = {0, 127, 255}));
  connect(teeJunctionIdeal3.port_3, volume1.ports[1]) annotation(
    Line(points = {{-6, 76}, {-6, 72}, {-2, 72}, {-2, 66}}, color = {0, 127, 255}));
  annotation (experiment(StopTime = 600, Interval = 0.25, Tolerance = 1e-6));
end HydrogenSubsystemTEST_simplifiedV9;
