...
model RecirculationBlowerTest "Simple model to test the recirculation blower model"
  extends Modelica.Icons.Example;
  inner Modelica.Fluid.System system annotation(
    Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  replaceable package Medium = Modelica.Media.IdealGases.SingleGases.H2(Temperature(start = system.T_start), AbsolutePressure(start = system.p_start * 3));
...
equation
  connect(boundary.ports[1], recirculationBlower.Input) annotation(
    Line(points = {{-60, 0}, {-18, 0}}, color = {0, 170, 0}, thickness = 1));
